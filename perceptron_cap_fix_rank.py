...
def rank_func_f():
    """
    Generic calculation of rank, without low rank recontruction
    """
    f_list = [0.001,0.01,0.05] + [i for i in np.linspace(0.1,0.8,4)] + [0.95,0.99,0.999]
    N = 100
    P_list = [60,80,120]
    theory_line = [min(i,N) for i in P_list]
    ranks = np.zeros((len(P_list),len(f_list)))
    for i,P in enumerate(P_list):
        for j,f in enumerate(f_list):
            patt_c = make_patterns(N,P,cod=f)
            rank = LA.matrix_rank(patt_c)
            ranks[i,j] = rank
        
    plt.figure()
    plt.title(r'Rank of $\xi$',fontsize=18)
    for i,P in enumerate(P_list):
        plt.plot(f_list,ranks[i,:],'o-',label=r'$P={}$'.format(P))
        #plt.axhline(theory_line[i],linestyle='--')
...
#
#func_evaluate_capacity_fixrank(f=0.9)

#func_evaluate_capacity_fixrank_varycoding()
